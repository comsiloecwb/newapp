...
export function EventoForm({ evento, action, submitLabel, onDelete }: EventoFormProps) {
  ...
  async function handleDelete() {
    if (!onDelete || !confirm('Tem certeza que deseja excluir este evento?')) return;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-stone-300 text-sm">{published ? 'Publicado (visível no app)' : 'Rascunho (oculto no app)'}</span>
      </div>
